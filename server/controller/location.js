...
const retrieveStripeProductOrNull = async ({ productId, stripeAccount }) => {
  if (!productId) return null;
  try {
    return await stripe.products.retrieve(productId, {}, { stripeAccount });
  } catch (error) {
...
const retrieveStripePriceOrNull = async ({ priceId, stripeAccount }) => {
  if (!priceId) return null;
  try {
    return await stripe.prices.retrieve(priceId, {}, { stripeAccount });
  } catch (error) {
    if (isStripeMissingResourceError(error)) return null;
    throw error;
  }
...
const stripePriceMatchesMonthlyPlan = ({ price, amountInCents, currency }) => {
  if (!price) return false;
  const recurringInterval = `${price?.recurring?.interval || ''}`.trim().toLowerCase();
  const priceCurrency = `${price?.currency || ''}`.trim().toLowerCase();

  return (
    recurringInterval === 'month' &&
    Number(price?.unit_amount) === Number(amountInCents) &&
    priceCurrency === `${currency || 'usd'}`.trim().toLowerCase()
  );
};
...
const syncMembershipWithStripe = async ({ membership, location, currentUser }) => {
  ...
  for (let index = 0; index < normalizedMembership.plans.length; index += 1) {
    ...
    const amountInCents = Math.round(Number(resolvedPrice || 0) * 100);
    ...
    let stripeProduct = await retrieveStripeProductOrNull({
    ...
    const stripePrice = await retrieveStripePriceOrNull({
