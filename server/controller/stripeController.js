...
export const createConnectAccount = async (req, res, next) => {
  try {
    ...
    if (user.role !== 'team') {
      return next(createError(403, 'Only spa owners (team role) can connect Stripe accounts'));
    }
...
export const createAccountLink = async (req, res, next) => {
  try {
    ...
    if (!user || !user.stripe?.accountId) {
      return next(createError(400, 'No Stripe account found. Please create one first.'));
    }
    ...
    const accountLink = await stripe.accountLinks.create({
      ...
      refresh_url: refreshUrl || `${process.env.CLIENT_URL}/management?stripe=refresh`,
      return_url: returnUrl || `${process.env.CLIENT_URL}/management?stripe=success`,
...
export const getAccountStatus = async (req, res, next) => {
  try {
    ...
    user.stripe.onboardingCompleted = account.details_submitted && account.charges_enabled;
...
export const createPaymentIntent = async (req, res, next) => {
  try {
    ...
    if (!spaOwner.stripe?.accountId || !spaOwner.stripe?.chargesEnabled) {
      return next(createError(400, 'This spa has not connected their payment account yet'));
    }
    ...
    if (service.discount?.active && service.discount.percentage > 0) {
      const now = new Date();
      const startDate = service.discount.startDate ? new Date(service.discount.startDate) : new Date();
      const endDate = service.discount.endDate ? new Date(service.discount.endDate) : new Date();
  ...
  try {
    ...
    if (paymentIntent.status === 'succeeded') {
      ...
      payment.paymentMethod = {
        ...
        expMonth: paymentIntent.charges?.data[0]?.payment_method_details?.card?.exp_month || null,
        expYear: paymentIntent.charges?.data[0]?.payment_method_details?.card?.exp_year || null,
      };
...
export const getPaymentHistory = async (req, res, next) => {
  try {
    ...
    const result = await Payment.getCustomerHistory(userId, parseInt(page), parseInt(limit));
...
export const getRevenueAnalytics = async (req, res, next) => {
  try {
    ...
    if (user.role !== 'team') {
      return next(createError(403, 'Only spa owners can access revenue analytics'));
    }
