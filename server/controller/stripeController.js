...
const MEMBERSHIP_DELINQUENT_STATUSES = new Set(['past_due', 'unpaid'])
...
const getSubscriptionPeriodUnix = (subscription = null, boundary = 'end') => {
  const subscriptionKey =
    boundary === 'start' ? 'current_period_start' : 'current_period_end'
  const phaseKey = boundary === 'start' ? 'start_date' : 'end_date'
  const linePeriodKey = boundary === 'start' ? 'start' : 'end'

  const candidates = [
    subscription?.[subscriptionKey],
    subscription?.items?.data?.[0]?.[subscriptionKey],
    subscription?.current_phase?.[phaseKey],
    subscription?.latest_invoice?.lines?.data?.[0]?.period?.[linePeriodKey],
  ]

  for (const candidate of candidates) {
    const timestamp = getUnixTimestamp(candidate)
...
const getSubscriptionPeriodDate = (subscription = null, boundary = 'end') => {
  const timestamp = getSubscriptionPeriodUnix(subscription, boundary)
  return timestamp ? toDateFromUnix(timestamp) : null
}

const getSubscriptionScheduleId = (subscription = null) =>
...
const releaseMembershipSubscriptionSchedule = async ({
  scheduleId,
  stripeAccountId,
}) => {
  ...
  try {
    return await stripe.subscriptionSchedules.release(
      scheduleId,
      {},
    ...
    const isAlreadyGone =
      ...
      releaseError?.code === 'resource_missing'
    if (!isAlreadyGone) {
      throw releaseError
    }
    return null
...
const enforceMembershipGraceExpiryForUser = async ({
  user,
  stripeAccountId = null,
  subscription = null,
  now = new Date(),
  ...
  const status = `${subscription?.status || user.membershipBilling?.subscriptionStatus || ''}`
    .trim()
    .toLowerCase()
  if (!MEMBERSHIP_DELINQUENT_STATUSES.has(status)) {
    return false
  }

  const gracePeriodEndsAt = user.membershipBilling?.gracePeriodEndsAt
    ? new Date(user.membershipBilling.gracePeriodEndsAt)
    : null
  if (
    !gracePeriodEndsAt ||
    Number.isNaN(gracePeriodEndsAt.getTime()) ||
  ...
  if (subscriptionId && resolvedStripeAccountId) {
    try {
      latestSubscription = await stripe.subscriptions.retrieve(
        subscriptionId,
  ...
  const latestStatus = `${latestSubscription?.status || status}`.trim().toLowerCase()
  if (MEMBERSHIP_ACTIVE_STATUSES.has(latestStatus)) {
    await applyMembershipStateToUser({
      user,
      locationId: user.membershipBilling?.locationId || user.membership?.locationId || null,
      serviceId: user.membershipBilling?.serviceId || user.membership?.serviceId || null,
      subscription: latestSubscription,
      plan: null,
      stripeAccountId: resolvedStripeAccountId,
      defaultPaymentMethod:
        latestSubscription?.default_payment_method &&
        typeof latestSubscription.default_payment_method !== 'string'
          ? latestSubscription.default_payment_method
          : null,
  ...
  if (subscriptionId && resolvedStripeAccountId && latestSubscription) {
    try {
      ...
      const isAlreadyGone =
        cancelError?.type === 'StripeInvalidRequestError' &&
        cancelError?.code === 'resource_missing'
      if (!isAlreadyGone) {
        throw cancelError
      }
    }
...
export const processOverdueMembershipPaymentFailures = async ({
  now = new Date(),
  limit = 100,
} = {}) => {
  const users = await User.find({
    isDeleted: { $ne: true },
    'membershipBilling.subscriptionStatus': {
      $in: Array.from(MEMBERSHIP_DELINQUENT_STATUSES),
    },
    'membershipBilling.gracePeriodEndsAt': { $lte: now },
    'membershipBilling.subscriptionId': { $nin: [null, ''] },
    'membershipBilling.stripeAccountId': { $nin: [null, ''] },
  }).limit(limit)
  ...
  for (const user of users) {
    checked += 1
    try {
      const didDowngrade = await enforceMembershipGraceExpiryForUser({
        user,
        stripeAccountId: user.membershipBilling?.stripeAccountId || null,
        now,
      })
      if (didDowngrade) downgraded += 1
    } catch (error) {
      failed += 1
      console.error('Failed processing overdue membership payment:', {
        userId: `${user?._id || ''}`,
  ...
  return { checked, downgraded, failed }
}
...
const resolveBillableMembershipPlanForStripeAccount = async ({
  ...
  try {
    ...
    if (`${stripePrice?.recurring?.interval || ''}`.trim().toLowerCase() !== 'month') {
      throw createError(
        400,
        'This membership plan must use a monthly recurring Stripe price.'
      )
    }
    const stripeUnitAmount = Number(stripePrice?.unit_amount)
    return {
      ...plan,
      numericPrice: Number.isFinite(stripeUnitAmount)
        ? stripeUnitAmount / 100
        : plan.numericPrice,
      currency: `${stripePrice?.currency || plan.currency || 'usd'}`.toLowerCase(),
      stripeUnitAmount: Number.isFinite(stripeUnitAmount) ? stripeUnitAmount : null,
      stripeRecurringInterval: `${stripePrice?.recurring?.interval || ''}`
        .trim()
        .toLowerCase(),
    }
  } catch (error) {
...
const resolveMembershipPlanFromStripeSubscription = ({ location, subscription }) => {
  const price = subscription?.items?.data?.[0]?.price || null
  const priceId = price?.id || null
  const configuredPlan = resolveMembershipPlanFromPriceId({ location, priceId })
  if (configuredPlan) return configuredPlan

  const productName =
    price?.product && typeof price.product !== 'string' ? price.product.name : null
  const unitAmount = Number(price?.unit_amount)

  if (!priceId && !productName && !Number.isFinite(unitAmount)) {
    return null
  }

  return {
    name: price?.nickname || productName || 'Membership',
    resolvedPlanId: priceId,
    numericPrice: Number.isFinite(unitAmount) ? unitAmount / 100 : null,
...
const applyMembershipStateToUser = async ({
  ...
  const currentPeriodStart =
    getSubscriptionPeriodDate(subscription, 'start') ||
    (subscription ? null : user.membershipBilling?.currentPeriodStart || null)
  const currentPeriodEnd =
    getSubscriptionPeriodDate(subscription, 'end') ||
    (subscription ? null : user.membershipBilling?.currentPeriodEnd || null)
...
export const getMembershipBillingSummary = async (req, res, next) => {
  try {
    ...
    if (user.membershipBilling?.subscriptionId && stripeAccountId) {
      try {
        ...
        const summaryLocation = await Location.findOne({ locationId })
        const subscriptionPlan = resolveMembershipPlanFromStripeSubscription({
        ...
        if (scheduleId) {
          try {
            ...
            isScheduledToCancel =
              `${schedule?.end_behavior || ''}`.trim().toLowerCase() === 'cancel'
          } catch (scheduleRefreshError) {
            console.error('Failed to refresh membership subscription schedule:', scheduleRefreshError)
          }
        }
        ...
        await applyMembershipStateToUser({
          ...
          defaultPaymentMethod:
            ...
            typeof subscription.default_payment_method !== 'string'
              ? subscription.default_payment_method
              : null,
          preserveCurrentPlan: !subscriptionPlan,
        })

        if (isScheduledToCancel) {
          const membershipEndsAt = getSubscriptionPeriodDate(subscription, 'end')
          user.set('membershipBilling.cancelAtPeriodEnd', true)
          if (membershipEndsAt) {
            user.set('membershipBilling.currentPeriodEnd', membershipEndsAt)
            user.membership = {
              ...(user.membership || {}),
        ...
        await enforceMembershipGraceExpiryForUser({
          user,
          stripeAccountId,
          subscription,
        })
      } catch (subscriptionRefreshError) {
        console.error('Failed to refresh membership subscription:', subscriptionRefreshError)
      }
    }
...
export const removeMembershipPaymentMethod = async (req, res, next) => {
  try {
    ...
    const hasSubscriptionRequiringCard =
      user.membershipBilling?.subscriptionId &&
      !user.membershipBilling?.cancelAtPeriodEnd &&
...
export const changeMembershipSubscriptionPlan = async (req, res, next) => {
  try {
    ...
    const subscriptionItemId = subscriptionItem?.id || user.membershipBilling?.subscriptionItemId || null
    if (!currentPriceId) {
      return next(
        createError(
          400,
    ...
    if (!subscriptionItemId) {
      return next(
        createError(
          400,
          'Current membership subscription item could not be resolved.'
        )
      )
    }
    ...
    const rawCurrentStripeUnitAmount = subscriptionItem?.price?.unit_amount
    const currentStripeUnitAmount = Number(rawCurrentStripeUnitAmount)
    const currentPlan = resolveMembershipPlanFromPriceId({ location, priceId: currentPriceId })
    const currentPlanPrice = Number(
      rawCurrentStripeUnitAmount !== null &&
        rawCurrentStripeUnitAmount !== undefined &&
        Number.isFinite(currentStripeUnitAmount)
        ? currentStripeUnitAmount / 100
        : currentPlan?.numericPrice ?? user.membership?.price ?? 0
    )
    const rawTargetStripeUnitAmount = plan?.stripeUnitAmount
    const targetStripeUnitAmount = Number(rawTargetStripeUnitAmount)
    const targetPlanPrice = Number(
      rawTargetStripeUnitAmount !== null &&
        rawTargetStripeUnitAmount !== undefined &&
        Number.isFinite(targetStripeUnitAmount)
        ? targetStripeUnitAmount / 100
        : plan?.numericPrice || 0
    )
    const currentComparableAmount =
      rawCurrentStripeUnitAmount !== null &&
      rawCurrentStripeUnitAmount !== undefined &&
      Number.isFinite(currentStripeUnitAmount)
      ? currentStripeUnitAmount
      : Math.round(currentPlanPrice * 100)
    const targetComparableAmount =
      rawTargetStripeUnitAmount !== null &&
      rawTargetStripeUnitAmount !== undefined &&
      Number.isFinite(targetStripeUnitAmount)
      ? targetStripeUnitAmount
      : Math.round(targetPlanPrice * 100)
    const isUpgrade = targetComparableAmount > currentComparableAmount
    const subscriptionQuantity = Number(subscriptionItem?.quantity || 1)
    const normalizedQuantity = Number.isFinite(subscriptionQuantity)
    ...
    if (isUpgrade) {
      ...
      if (scheduleId) {
        await releaseMembershipSubscriptionSchedule({ scheduleId, stripeAccountId })
      }
      ...
      for (let attempt = 0; attempt < 2; attempt += 1) {
        try {
          ...
          if (
            attempt === 0 &&
            !activePlan?.isEphemeralTestPrice &&
            isStripeMissingPriceError(subscriptionUpdateError) &&
            canAutoCreateMembershipTestPrice()
          ) {
            activePlan = await createEphemeralMembershipTestPrice({
              plan: activePlan,
      ...
      if (!updatedSubscription) {
        throw createError(500, 'Unable to upgrade membership plan at the moment.')
      }
      ...
      res.status(200).json({
        success: true,
        message: 'Membership upgraded immediately with prorated billing',
        changeType: 'upgrade',
        ...
        invoiceStatus:
          typeof updatedSubscription.latest_invoice === 'string'
            ? null
            : updatedSubscription.latest_invoice?.status || null,
      })
      return
    }

    const currentPeriodEndUnix = getSubscriptionPeriodUnix(subscription, 'end') || 0
    ...
    const currentPeriodStartUnix = Number(
      getSubscriptionPeriodUnix(subscription, 'start') || Math.floor(Date.now() / 1000)
    )
    ...
    const scheduledChangeType =
      targetComparableAmount < currentComparableAmount ? 'downgrade' : 'scheduled'
    res.status(200).json({
      success: true,
      message:
        scheduledChangeType === 'downgrade'
          ? 'Membership downgrade scheduled for your next renewal'
...
export const cancelMembershipSubscription = async (req, res, next) => {
  try {
    const user = await User.findById(req.user.id)
    ...
    const stripeAccountId = user.membershipBilling.stripeAccountId
    const subscriptionId = user.membershipBilling.subscriptionId
    ...
    if (!MEMBERSHIP_VISIBLE_STATUSES.has(subscriptionStatus)) {
      return next(createError(400, 'No active membership subscription found'))
    }
    ...
    if (!Number.isFinite(currentPeriodEndUnix) || currentPeriodEndUnix <= 0) {
      return next(
        createError(
          400,
          'Unable to schedule this cancellation because the current billing period is unknown.'
        )
      )
    }
            ...
            phases: [
              {
                ...
                items: [
                  {
                    price: currentPriceId,
                    quantity: Number.isFinite(subscriptionQuantity)
                      ? subscriptionQuantity
                      : 1,
                  },
                ],
    ...
    const membershipEndsAt = getSubscriptionPeriodDate(subscription, 'end') || toDateFromUnix(currentPeriodEndUnix)
    user.set('membershipBilling.cancelAtPeriodEnd', true)
    user.set('membershipBilling.currentPeriodEnd', membershipEndsAt)
    user.set('membershipBilling.pendingPlan', getEmptyMembershipPendingPlan())
    user.membership = {
      ...(user.membership || {}),
      expiresAt: membershipEndsAt || user.membership?.expiresAt || null,
    }
    user.activeMembership = {
      ...(user.activeMembership || {}),
      expiresAt: membershipEndsAt || user.activeMembership?.expiresAt || null,
    }
    await user.save()
    ...
    res.status(200).json({
      ...
      membershipEndsAt: user.membershipBilling?.currentPeriodEnd || null,
    })
  } catch (error) {
