...
export const shouldConsiderBookingConflict = (
  booking,
  { ignorePendingHoldsForUserId = null } = {}
) => {
  ...
  if (paymentStatus === 'pending') {
    const bookingUserId = `${booking.userId?._id || booking.userId || ''}`.trim()
    const ignoredUserId = `${ignorePendingHoldsForUserId || ''}`.trim()
    if (ignoredUserId && bookingUserId && bookingUserId === ignoredUserId) {
      return false
    }
...
export const getDailySchedulingContext = async ({
  ...
  const rawBookings = await Booking.find(query)
    .select('date time duration serviceId userId ghl status paymentStatus stripeSessionId createdAt')
    .populate('serviceId', 'ghlCalendar')

  const relevantBookings = rawBookings.filter(
    (booking) =>
      isBookingRelevantForService(booking, service) &&
      shouldConsiderBookingConflict(booking, { ignorePendingHoldsForUserId })
  )
