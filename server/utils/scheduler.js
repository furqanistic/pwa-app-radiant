...
import { processOverdueMembershipPaymentFailures } from '../controller/stripeController.js';
...
export const initScheduler = () => {
  ...
  // Membership failed-payment grace check every hour.
  cron.schedule('0 * * * *', async () => {
    console.log('Running overdue membership payment check...')
    try {
      const result = await processOverdueMembershipPaymentFailures()
      if (result.checked > 0 || result.failed > 0) {
        console.log(
          `Membership payment check complete: ${result.checked} checked, ${result.downgraded} downgraded, ${result.failed} failed.`
        )
      }
    } catch (error) {
      console.error('Error running overdue membership payment check:', error)
    }
  })
