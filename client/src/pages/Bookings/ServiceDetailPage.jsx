...
const ServiceDetailPage = () => {
  ...
  const toastSuccess = useCallback((message, options = {}) =>
    toast.success(message, { ...toastStyle, ...options }), [toastStyle]);
  const toastError = useCallback((message, options = {}) =>
    toast.error(message, { ...toastStyle, ...options }), [toastStyle]);
  ...
  const { 
    ...
    selectedDate,
    serviceId,
    { enabled: !isProcessing && !checkoutOpen }
  );
  ...
  }, [selectedTime, availableTimes, loadingAvailability, isProcessing, checkoutOpen, toastError]);
  ...
  }, [service, selectedTreatments.length]);
                        ...
                        <>
                          ...
                          {!hasSavedMembershipCard && (
                            <button
                              ...
                              disabled={isProcessing}
                              className="w-full py-3 bg-white border border-gray-200/70 text-gray-800 rounded-xl font-semibold hover:bg-gray-50 transition-all flex items-center justify-center gap-2 disabled:cursor-not-allowed disabled:opacity-50"
                            >
                      ...
                      <Plus className="mx-auto h-5 w-5" />
	                   ...
	                   {!hasSavedMembershipCard && (
	                     <button
	                        onClick={() => setAddCardDialogOpen(true)}
	                        disabled={isProcessing}
	                        className="h-12 w-12 rounded-xl border border-slate-200 bg-slate-50 text-slate-600 transition-colors hover:bg-slate-100 active:scale-[0.98] disabled:cursor-not-allowed disabled:opacity-50"
	                        aria-label="Add card"
	                     >
