...
import { buildSubdomainUrl, getCurrentSubdomain } from '@/utils/subdomain'
...
const AuthPage = () => {
  ...
  const fallbackBusinessName = 'CxR systems'
  const businessName = branding?.name?.trim() || fallbackBusinessName
  ...
  const handleSpaSelection = useCallback(
    (spa) => {
      if (!spa?.locationId) return

      const normalizedSubdomain = `${spa.subdomain || ''}`.trim().toLowerCase()
      const hasValidSubdomain =
        normalizedSubdomain &&
        !['app', 'www', 'api', 'localhost', '127'].includes(normalizedSubdomain)

      if (hasValidSubdomain && typeof window !== 'undefined') {
        const targetBaseUrl = buildSubdomainUrl(normalizedSubdomain)
        const targetHost = `${normalizedSubdomain}.cxrsystems.com`
        ...
        if (
          ...
          (currentHost === 'app.cxrsystems.com' ||
            currentHost === 'www.cxrsystems.com' ||
            currentHost === 'cxrsystems.com')
        ) {
      ...
      navigate(`/auth?spa=${encodeURIComponent(spa.locationId)}`)
    },
    [navigate]
  )
  ...
  const signupMutation = useMutation({
    ...
    onSuccess: async (data) => {
      ...
      setSuccess(
        `${businessName} account created! Transforming your experience...`
      )
      const targetPath =
