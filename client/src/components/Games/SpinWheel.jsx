...
const SpinWheel = ({ game, onPlay, isPlaying, canPlay }) => {
  ...
  // Vibrant, harmonious colors for the wheel segments
  const colors = [
    '#ec4899', // Pink 500
    '#f43f5e', // Rose 500
    '#d946ef', // Fuchsia 500
  ...
  return (
    <div className='flex flex-col items-center py-4'>
      ...
      <div className='relative z-20 translate-y-4 filter drop-shadow-lg'>
        <div className='w-8 h-8 bg-white rotate-45 transform origin-center border-4 border-pink-500 rounded-sm shadow-md'></div>
      </div>

      {/* Wheel Container with Shadow/Depth */}
      <div className='relative w-72 h-72 md:w-80 md:h-80 rounded-full bg-white p-2 shadow-2xl shadow-pink-500/20'>
        <div className='w-full h-full rounded-full overflow-hidden relative border-4 border-pink-100'>
          <motion.div
        ...
        {/* Center Cap */}
        <div className='absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-16 h-16 bg-white rounded-full flex items-center justify-center shadow-lg border-4 border-pink-100 z-10'>
           <motion.div 
             animate={isSpinning ? { scale: [1, 1.1, 1] } : {}}
             transition={{ duration: 0.5, repeat: Infinity }}
             className="w-10 h-10 rounded-full bg-gradient-to-br from-pink-500 to-rose-600 flex items-center justify-center"
           >
      ...
      <div className='mt-8 w-full max-w-xs'>
        <button
          ...
          className='w-full py-4 relative group overflow-hidden rounded-2xl bg-gradient-to-r from-pink-500 to-rose-600 text-white shadow-xl shadow-pink-500/30 hover:scale-[1.02] active:scale-[0.98] transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100'
        >
