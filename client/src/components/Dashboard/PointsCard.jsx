...
const PointsCard = () => {
  ...
  return (
    ...
    >
      ...
      <div className='absolute -inset-2 bg-gradient-to-r from-pink-400 via-rose-300 to-pink-400 rounded-[2.5rem] blur-2xl opacity-30 group-hover:opacity-50 transition-all duration-700'></div>

      <motion.div
        whileHover={{ y: -8, rotateX: 2, rotateY: -2 }}
        className='relative aspect-[1.6/1] md:aspect-auto md:min-h-64 rounded-[2rem] overflow-hidden bg-pink-500 border border-white/30 shadow-2xl'
      >
        ...
        <div className='relative h-full p-8 flex flex-col justify-between z-10'>
          ...
          <div className='flex justify-between items-start'>
            ...
            <div className='relative w-14 h-10 rounded-lg bg-white/20 border border-white/40 backdrop-blur-md shadow-inner overflow-hidden'>
              <div className='absolute inset-0 grid grid-cols-3 grid-rows-3 gap-0.5 p-1.5 opacity-40'>
                {[...Array(9)].map((_, i) => (
                  <div key={i} className='border-[1px] border-white rounded-[2px]' />
                ))}
              </div>
            </div>
          </div>
          ...
          <div className='flex flex-col relative mt-2'>
            ...
            <div className='flex items-baseline gap-2'>
              <motion.span 
                ...
                className='text-5xl md:text-6xl font-black text-white drop-shadow-[0_4px_12px_rgba(0,0,0,0.1)] tracking-tighter'
              >
                {currentUser?.points?.toLocaleString() || '0'}
              </motion.span>
              <span className='text-sm font-bold text-white/90 tracking-[0.2em] uppercase drop-shadow-sm'>Points</span>
            </div>
          ...
          <div className='flex justify-between items-end mt-4'>
            <div className='flex flex-col gap-1'>
              <span className='text-[9px] font-black text-white/70 uppercase tracking-[0.2em]'>Cardholder</span>
              <span className='text-lg font-bold text-white tracking-wide drop-shadow-sm'>
                {currentUser?.name || 'Radiant Member'}
              </span>
            </div>

            <div className='flex items-center gap-5 bg-white/20 backdrop-blur-3xl rounded-2xl px-5 py-3 border border-white/30 shadow-xl'>
              <div className='flex flex-col items-end'>
                <span className='text-[8px] font-black text-white/70 uppercase tracking-[0.3em]'>Joined</span>
